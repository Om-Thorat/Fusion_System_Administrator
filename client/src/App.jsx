...
function Layout({ toggleColorScheme, colorScheme }) {
  return (
    <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden" }}>
      <Sidebar toggleColorScheme={toggleColorScheme} colorScheme={colorScheme} />
      <div style={{ flex: 1, overflowY: "auto", paddingRight: "80px" }}>
        <Routes>
          ...
          <Route path="/backups/schedules" element={<RequireAuth><SchedulePage /></RequireAuth>} />
        </Routes>
      </div>
    </div>
  );
...
function App() {
  ...
  return (
    <MantineProvider theme={createTheme({ colorScheme })} forceColorScheme={colorScheme}>
      <Notifications />
      <AuthProvider>
        <Router>
          <Routes>
